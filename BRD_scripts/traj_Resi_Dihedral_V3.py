...
Pdy_phi = univ.selectAtoms( 'resnum ' + str(i_res-3) + ' and name C',
                            'resnum ' + str(i_res-2) + ' and name N',
                            'resnum ' + str(i_res-2) + ' and name CA',
                            'resnum ' + str(i_res-2) + ' and name C');  #C-1-N-CA-C
Pdy_psi = univ.selectAtoms( 'resnum ' + str(i_res-2) + ' and name N',
                            'resnum ' + str(i_res-2) + ' and name CA',
                            'resnum ' + str(i_res-2) + ' and name C',
                            'resnum ' + str(i_res-1) + ' and name N'); #N-CA-C-N+1
Pdy_chi = univ.selectAtoms( 'resnum ' + str(i_res-2) + ' and name N',
                             'resnum ' + str(i_res-2) + ' and name CA',
                             'resnum ' + str(i_res-2) + ' and name CB',
                             'resnum ' + str(i_res-2) + ' and name CG'); #N-CA-CB-CG
...
Yn_O = univ.selectAtoms( 'resnum ' + str(k_res) + ' and name CA',
                            ...
                            'resnum ' + str(k_res) + ' and name HH',
                            'resnum ' + str(k_res) + ' and name OH' ); # CA-CB-OH-HH
...
print >>ofile, 'frame'.ljust(10) + 'pdY_phi pdY_psi pdY_O pdY_chi1 Pdy_phi Pdy_psi Pdy_chi  N1_ND2 Yn_O Yn_chi1  wPf_phi wPf_psi pMd_phi pMd_psi preP_phi preP_psi';
for ts in univ.trajectory:
    print >>ofile, str(ts.frame).ljust(10) + ' ' + str(pdY_phi.dihedral()) + ' ' + str(pdY_psi.dihedral()) + ' ' + str(pdY_O.dihedral())+\
  	 ' ' + str(pdY_chi.dihedral()) + ' '+str(Pdy_phi.dihedral())+' '+str(Pdy_psi.dihedral())+' '+str(Pdy_chi.dihedral())+' ' +\
	 str(N1_ND2.dihedral()) + ' ' + str(Yn_O.dihedral()) + ' ' + str(Yn_chi.dihedral()) + ' ' + str(wPf_phi.dihedral()) + ' ' + str(wPf_psi.dihedral())+ \
