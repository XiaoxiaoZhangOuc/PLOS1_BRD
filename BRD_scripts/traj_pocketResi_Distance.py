...
mark3 = univ.selectAtoms( 'resid ' + str( int( pro_marks[pro][2] )+1 ) + ' and name OH')[0];
# resi pdY and name OH
mark4 = univ.selectAtoms( 'resid ' + pro_marks[pro][3] + ' and name O' )[0];
# resi pMd and name O
mark5 = univ.selectAtoms( 'resid ' + str( int( pro_marks[pro][6] ) - 7 ) + ' and name O' )[0];
# resi N1-3  Valine O
mark6 = univ.selectAtoms( 'resid ' + str( int( pro_marks[pro][6] ) - 4 ) + ' and name ND2' )[0];
# resi N1 and name ND2
mark7 = univ.selectAtoms( 'resid ' + str( int( pro_marks[pro][6] ) - 4 ) + ' and name O' )[0];
# resi N1 and name O
...
ofile1 = file( pro_+'_pocket_marker_distance.txt', 'w');
ofile1.write( 'frame'.ljust(8) + ' wPf-preP   wPf-pMd   preP-pMd   pMd-pdY   wPf-pdY   preP-pdY   pMd-V   pMd-N1_ND' + \
              '   pdY-N1    wPf-V\n' );
...
for ts in univ.trajectory:
    ...
    ofile1.write( '  ' + '%8.2f' %lg.norm(mark2.position-mark4.position) );
    ofile1.write( '  ' + '%8.2f' %lg.norm(mark4.position-mark3.position) );

    ofile1.write( '  ' + '%8.2f' %lg.norm(mark1.position-mark3.position) );
    ofile1.write( '  ' + '%8.2f' %lg.norm(mark2.position-mark3.position) );
